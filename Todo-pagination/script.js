;(function () {
  ...
  let currentPage = 1 //나중에 현재페이지 값 저장할것
  const totalCount = 53 //임의의 값 설정이 필요해서 53을 넣어줌 
  const pageCount = 5
  const limit = 5
...
//현재 페이지 그룹의 첫번째 숫자 구하기
    ...
    const next = lastNumber + 1 //다음을 눌렀을때 몇번째인지 나타냄 
    const prev = firstNumber - 1 //이전을 눌렀을때 어디로 이동할지 나타냄
    ...
    $currentPageNumbers.forEach((button) => {
      button.addEventListener('click', () => {
        if (button.dataset.fn === 'prev') {
          ...
          currentPage = button.innerText;  //{i}가 된다 
        }
        //눌러서 값들이 바뀌면 다시 한번 실행해야하기때문에 아래 함수들 호출
        pagination() 
        getTodos()
      })
    })
